...
u8 Image_Data[120][120];      //图像原始数据存放
...
{
  if(over)                                 //完成一场图像采集后显示并发送数据到上位机
  {
    Get_Use_Image1(Field_Over_Flag-1);     //采集图像数据存放数
    //    if(DialSwitch_5)
     //   {
    //     串口发送数据非常慢，注释掉OLED刷新很快
    //     DeEXTI_Init(PTD,14); //行中断关闭
    //     DeEXTI_Init(PTD,15); //场中断关闭       
   ...
   // if(DialSwitch_1==1) 
   // {
    // TFTSPI_Show_Pic4(0, 0, 120, 120, Image_Use); //龙邱TFT1.8显示160*120灰度图像Image_Use    0, 0, 160, 120,Image_Use            
 //   }
    Get_01_Value();                  //二值化图像数
    //JZTX();                          //校正图像
   // Pixle_Filter();                  //去噪点    
  //  if(DialSwitch_1==1&&DialSwitch_2==1) 
    //{
      TFTSPI_Show_Pic3(0, 0, 120, 120, Image_Use); //龙邱TFT1.8显示160*120灰度图像Image_Use    0, 0, 160, 120,Image_Use            
    //} 
    over= 0;       
  }
}
...
void ULTRASOUND(void)
{
  ...
  //  pit_time_start(PIT1);
  /* while(GPIO_Get(PTB1));       
  time=pit_time_get(PIT1);  //读取高电平时间，即超声波往返时间
  dis=time/58.0;            //计算距离dis
  DIS=(int)dis/100;
  */
}
...
{
  if((DIS<85)&&guole==0&&broken_p==1&&DIS!=0&&ABS(Error)<20)   //避障&&broken_p==1
  {
...
{
  //if(DialSwitch_3)
    MotorControlOpenLoop();//开环控制
  //else
    //MotorControl();//增量式pid算法
}
...
{     
  ...
  {    
    ...
    // 用户程序            
    //DMA采集，在这里修改采集接口PTD_BYTE0_IN是0--D7。   PLCK接的是PTD13。   
...
void SendPicture(void)
{
  ...
  for(i=0;i<CAMERA_H;i++)      //输出
  {
    for(j=0;j<CAMERA_W;j++)    //输出从第0列到列，用户可以选择性的输出合适的列数
    {
...
{     
  ...
  //摄像头寄存器设置
  SCCB_Init(); 
  //两个地址口都拉高，为MT9V034_I2C_ADDR  
  
  MT9V034_SetFrameResolution(CAMERA_H, CAMERA_W);//设置摄像头图像4*4分频输出PCLK, 27/4 = 6.75M ,BIT4,5镜像设置:上下左右均镜像   
...
__ramfunc void Get_Use_Image1(u8 picture_number)
{
  ...
  if(picture_number)
  {
    for(i = 0; i  < CAMERA_H; i+=1)  //120行，每2行采集一行，
    {
...
void Get_01_Value(void)
{
 ...
 /* u32 tv=0;
  ...
  GaveValue=25*tv/(LCDH)/(LCDW);     //求平均值,光线越暗越小，全黑约35，对着屏幕约160，一般情况下大约100 
  // sprintf(txt,"%03d:%03d",Threshold,GaveValue);//前者为大津法求得的阈值，后者为平均值  
  //LCD_P6x8Str(80,1,(u8*)txt);
  //按照均值的比例进行二值化*/
 // GaveValue=GaveValue*7/10+20;//*7/10+2;//此处阈值设置，根据环境的光线来设定   实验室白天为0.9或1，晚上为0.3或0.4
...
/*************************************************************** 
* 
* 函数名称：uint8_t GetOSTU(uint8_t tmImage[CAMERA_H][CAMERA_W]) 
* 功能说明：求阈值大小 
* 参数说明： 
* 函数返回：阈值大小 
* 修改时间：2018年3月27日 
...
/*************************************************************** 
* 
* 函数名称：void BinaryImage(uint8_t tmImage[CAMERA_H][CAMERA_W]) 
* 功能说明：图像数据二值化 
* 参数说明： 
* 函数返回：void 
* 修改时间：2018年3月27日 
