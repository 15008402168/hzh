...
{ 
  ...
  while(Del--)
  {	
    for(i=0;i<50;i++) asm("NOP");//RT1052 528M: 30000是1ms,30是1M
  }
}
...
/****************显示图片（从单片机的DATA区）********************************/	
void TFTSPI_Show_pic()
{
  unsigned int i,j,k=0;
  
  /*	TFTSPI_Write_Cmd(0x2a);                //Horizontal Address Start Position
  TFTSPI_Write_Byte(0x00);
  TFTSPI_Write_Byte(0x00);
  TFTSPI_Write_Byte(0x00);
  TFTSPI_Write_Byte(0xa2);		       //0--127水平地址结束  a8
  TFTSPI_Write_Cmd(0x2b);                     //Vertival Address end Position
  TFTSPI_Write_Byte(0x00);
  TFTSPI_Write_Byte(0x00);
  TFTSPI_Write_Byte(0x00);
  TFTSPI_Write_Byte(0x84);		       //0--159，垂直地址结  b3
  TFTSPI_Write_Cmd(0x2c);
  */	
  //TFTSPI_Set_Pos(0,0,TFT18W,TFT18H);
  TFTSPI_Set_Pos(0,0,TFT18W-1,TFT18H);
  for(i=0;i<TFT18H;i++)
  { 
    for(j=0;j<TFT18W;j++)
    {			
      //TFTSPI_Write_Word(((uint16_t)MYPIC[k])<<8|MYPIC[k+1]);
      TFTSPI_Write_Word(((uint16_t)u16ftflogo[k])<<8|u16ftflogo[k+1]);
      //TFTSPI_Write_Word(ftflogo[k]<<8|ftflogo[k+1]);
      k++; k++;
    }
  }    
}
...
//x: 0 - 7	(行)
//y: 0 -12	(列)
void TFTSPI_P16x12Str(unsigned char x,unsigned char y,uint8_t *s_dat,uint16_t word_color,uint16_t back_color)
{
...
//x: 0 - 7	(行)
//y: 0 -9	(列)
void TFTSPI_P16x16Str(unsigned char x,unsigned char y,uint8_t *s_dat,uint16_t word_color,uint16_t back_color)
{
  unsigned char wm=0,ii = 0,i,j;
  unsigned int adder=1; 
