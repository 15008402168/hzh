...
#define CAMERA_H  120  //行 HEIGHT 待采集摄像头图像高度行数
#define CAMERA_W  160  //列 WIDTH  待采集摄像头图像宽度列数

#define LCDH    120  //OLED显示的行数
#define LCDW    120  //OLED显示的列数
...
void middlelineshow(uint8_t xs,uint8_t ys,uint8_t w,uint8_t h,uint8_t Use_Buff[LCDH][LCDW]);
...
void Get_Back(void);
void Draw_Road(void);
void Get_Use_Image1(u8 picture_number);
void Get_Use_Image2(void);
void Get_01_Value(void);
void Get_02_Value(void);
